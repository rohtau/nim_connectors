...
nimP.info("Flame Prefs: %s" % flamePrefFile)
...
try :
    ...
    if os.path.isdir(nimFlamePresetPath) == False :
        ...
        nimP.warning("NIM Preset folder for Flame major version %s not found.  Using default preset folder." % flame_major_version)
except Exception as e :
    nimP.warning ("Error while searching for export presets paths: %s.  Using default preset folder."%str(e))
    nimFlamePresetPath = os.path.join(nimFlamePresetPathBase,'_default')
...
try :
    ...
    nimP.info("NIM - Current Application: %s" % nimExport_app)
except :
    nimP.info("NIM - failed to load Wiretap API")
...
class rtExportSequenceDialog(QDialog):
    '''
    Main Export Dialog from Flame

    '''
    def __init__(self, parent=None):
        super(rtExportSequenceDialog, self).__init__(parent)
        ...
        try:
            ...
            nimP.info("Prefs successfully read")

        except:
            nimP.error("Failed to read NIM prefs")
            nimP.error('%s' % traceback.print_exc())
            self.app='Flame'
            self.user=''
            pass
        ...
        try:
            ...
            # failing on user
            nimP.error("Failed to get userID")
            self.nim_userID = 0

        nimP.info("NIM user=%s (%s)" % (self.user, self.nim_userID))
        # nimP.info("userID=%s" % self.nim_userID)
        ...
        self.setWindowTitle("rohtau: Export Sequence")
        # self.setStyleSheet("QLabel {font: 14pt}")
        ...
        jobIndex = 0
        jobIter = 0
        # Check if SHOW is defined, in that case then use it instead of allowing
        # selection
        env_job_index = -1
        env_job = ""
        if 'SHOW' in os.environ:
            env_job = os.getenv('SHOW')
        if len(self.nim_jobs)>0:
            for key, value in sorted(list(self.nim_jobs.items()), reverse=True):
                # self.nim_jobChooser.addItem(self.clearPix, key)
                ...
                if self.flamePrefs['jobID'] == value:
                    # print("Found matching Job Name, job=", key)
                    jobIndex = jobIter
                # Compare job with SHOW envar
                if key.startswith(env_job):
                    nimP.info("Current job found from environment: %s (%d)"%(key, jobIter))
                    env_job_index = jobIter
                jobIter += 1

            if self.flamePrefs['jobID'] != '':
                self.nim_jobChooser.setCurrentIndex(jobIndex)
            # force envar show if found
            if env_job_index > 0:
                nimP.info("Job set from environment to Index: %d"%env_job_index)
                self.nim_jobChooser.setCurrentIndex(env_job_index)
                self.nim_jobChooser.setEnabled(False)
        ...
        self.nim_serverChooser.setEnabled(False)
        ...
        # Do Publishing in NIM
        horizontalLayout_dopub = QHBoxLayout()
        horizontalLayout_dopub.setSpacing(-1)
        horizontalLayout_dopub.setSizeConstraint(QLayout.SetDefaultConstraint)
        horizontalLayout_dopub.setObjectName("horizontalLayout_dopub")
        self.nimDopubLabel = QLabel()
        self.nimDopubLabel.setFixedWidth(120)
        self.nimDopubLabel.setText("Publish Exports:")
        horizontalLayout_dopub.addWidget(self.nimDopubLabel)
        self.nim_dopubCheck = QCheckBox()
        self.nim_dopubCheck.setToolTip("Whether or not to publish export elements in NIM. This includes using auto versioning elements.")
        self.nim_dopubCheck.setCheckState(Qt.Checked)
        self.nim_dopubCheck.setMinimumHeight(28)
        self.nim_dopubCheck.setIconSize(QSize(1, 24))
        horizontalLayout_dopub.addWidget(self.nim_dopubCheck)
        self.nim_dopubCheck.stateChanged.connect(self.nim_dopubChanged)

        # Versioning
        horizontalLayout_version = QHBoxLayout()
        horizontalLayout_version.setSpacing(-1)
        horizontalLayout_version.setSizeConstraint(QLayout.SetDefaultConstraint)
        horizontalLayout_version.setObjectName("horizontalLayout_version")
        self.nimVersionLabel = QLabel()
        self.nimVersionLabel.setFixedWidth(120)
        self.nimVersionLabel.setText("Versioning:")
        horizontalLayout_version.addWidget(self.nimVersionLabel)
        self.nim_versionChooser = QComboBox()
        self.nim_versionChooser.setToolTip("Select versioning method, automatically Up Version or Manually set")
        self.nim_versionChooser.setMinimumHeight(28)
        self.nim_versionChooser.setIconSize(QSize(1, 24))
        self.nim_versionChooser.addItems(("Up Version", "Manual"))
        horizontalLayout_version.addWidget(self.nim_versionChooser)
        self.nimSetVersionLabel = QLabel()
        self.nimSetVersionLabel.setFixedWidth(120)
        self.nimSetVersionLabel.setText("Set Version:")
        horizontalLayout_version.addWidget(self.nimSetVersionLabel)
        self.nimVersionNumber = QSpinBox()
        self.nimVersionNumber.setRange(1, 100)
        self.nimVersionNumber.setEnabled(False)
        # self.nimSetVersionNumberLabel.setText("Set Version:")
        horizontalLayout_version.addWidget(self.nimVersionNumber)
        horizontalLayout_version.setStretch(1, 40)
        self.nim_versionChooser.currentIndexChanged.connect(self.nim_versionMethodChanged)
        self.nimVersionNumber.valueChanged.connect(self.nim_versionChanged)
        ...
        # Set by default to plates element type
        videoIndex = 0
        videoIter = 0
        plates_index = -1
        comps_index = -1
        for key, value in sorted(list(self.nim_elementTypesDict.items()), reverse=False):
            ...
            if self.flamePrefs['videoElementID'] == value:
                nimP.info("Found video element preference=", key)
                videoIndex = videoIter
            if key == 'plates':
                plates_index = videoIter
            elif key == 'comps':
                comps_index = videoIter
            videoIter += 1

        if self.flamePrefs['videoElementID'] != '':
            self.nim_videoChooser.setCurrentIndex(videoIndex)
        if plates_index > -1:
            self.nim_videoChooser.setCurrentIndex(plates_index)
            self.nim_videoChooser.setEnabled(False)
        ...
        for key, value in sorted(list(self.nim_elementTypesDict.items()), reverse=False):
            ...
            if self.flamePrefs['audioElementID'] == value:
                nimP.info("Found audio element preference=", key)
                audioIndex = audioIter
            audioIter += 1

        if self.flamePrefs['audioElementID'] != '':
            self.nim_audioChooser.setCurrentIndex(audioIndex)
        # If setting to plates element means we are doing automatic setup,
        # disable element
        if plates_index > -1:
            self.nim_audioChooser.setCurrentIndex(plates_index)
            self.nim_audioChooser.setEnabled(False)
        ...
        # Force element type to comps
        if comps_index > -1:
            self.nim_openClipChooser.setCurrentIndex(comps_index)
            self.nim_openClipChooser.setEnabled(False)

        self.nim_openClipChooser.currentIndexChanged.connect(self.nim_openClipElementChanged)

        # Timeline
        # By default export as plates
        # XXX: note sure what is the best way to publish timelines at the moment
        ...
        # Force element type to comps
        if comps_index > -1:
            self.nim_batchOpenClipChooser.setCurrentIndex(comps_index)
            self.nim_batchOpenClipChooser.setEnabled(False)
        ...
        # Use comp task
        if comp_task_idx > -1:
            self.nim_batchChooser.setCurrentIndex(comp_task_idx)
            self.nim_batchChooser.setEnabled(False)

        self.nim_batchChooser.currentIndexChanged.connect(self.nim_batchTaskChanged)


        # Add the standard ok/cancel buttons, default to ok.
        self._buttonbox = QDialogButtonBox( QDialogButtonBox.StandardButton.Help | QDialogButtonBox.StandardButton.Ok | QDialogButtonBox.StandardButton.Cancel)
        self._buttonbox.button(QDialogButtonBox.StandardButton.Help).setText("Export Report")
        self._buttonbox.button(QDialogButtonBox.StandardButton.Help).setToolTip("Show a report of what actions will be done in the next Export")
        ...
        self._buttonbox.helpRequested.connect(self.showExportReport)
        self._buttonbox.accepted.connect(self.acceptTest)
        self._buttonbox.rejected.connect(self.reject)
        horizontalLayout_OkCancel = QHBoxLayout()
        horizontalLayout_OkCancel.setSpacing(-1)
        ...
        groupLayout.setLayout(3, QFormLayout.SpanningRole, horizontalLayout_server)
        groupLayout.setLayout(4, QFormLayout.SpanningRole, horizontalLayout_show)
        groupLayout.setLayout(5, QFormLayout.SpanningRole, horizontalLayout_dopub)
        groupLayout.setLayout(6, QFormLayout.SpanningRole, horizontalLayout_version)

        '''
        groupLayout.setLayout(6, QFormLayout.SpanningRole, horizontalLayout_elementSpacer)
        groupLayout.setLayout(7, QFormLayout.SpanningRole, horizontalLayout_elementDesc)
        groupLayout.setLayout(8, QFormLayout.SpanningRole, horizontalLayout_video)
        groupLayout.setLayout(9, QFormLayout.SpanningRole, horizontalLayout_audio)
        groupLayout.setLayout(10, QFormLayout.SpanningRole, horizontalLayout_openClip)
        groupLayout.setLayout(11, QFormLayout.SpanningRole, horizontalLayout_batchOpenClip)

        groupLayout.setLayout(12, QFormLayout.SpanningRole, horizontalLayout_taskSpacer)
        groupLayout.setLayout(13, QFormLayout.SpanningRole, horizontalLayout_taskDesc)
        groupLayout.setLayout(14, QFormLayout.SpanningRole, horizontalLayout_batch)
        '''

        groupLayout.setLayout(15, QFormLayout.SpanningRole, horizontalLayout_OkCancel)
    ...
    def nim_dopubChanged(self, state):
        """Do NIM publishing chnaged changed"""
        # self.pub_dopub = state == QtCore.Qt.Checked
        self.pub_dopub = state == Qt.Checked
        if not self.pub_dopub:
            self.nim_versionChooser.setCurrentText('Manual')
        else:
            self.nim_versionChooser.setCurrentText('Up Version')

    def nim_versionMethodChanged(self):
        """Versioning method changed"""
        method = self.nim_versionChooser.currentText()
        self.pub_autover = method
        self.nimVersionNumber.setEnabled(method == 'Manual')

    def nim_versionChanged(self):
        """Version changed"""
        ver = self.nimVersionNumber.value()
        self.pub_ver = ver
    ...
    def showExportReport(self):
        '''
        Show Window with a report of the actions that will be taken in the next Export process
        ...
        '''
        nimP.info("Show Exports report windows. Not yet implemented :(")
...
def buildOutputPath( info, userData ):
    '''
    Using information passed by Flame in the info and userData dicts build output path.

    Parameters
    ----------
    info : dict
        Dict with the state of Flame export
    userData : dict
        Dict with info passed from one Flame export stage to the next
    
    Returns
    ---------
    str
        Output path for element or False if error.

    '''
    path = ""

    # Render name from track name
    name         = info['trackName']
    shot         = info['shotName']
    element_type = nimUtl.getelementsIDDict()[int(userData["%sElementID"%info['assetType']])]
    baseloc      = userData['nim_platesPath']
    if element_type == 'precomp' or element_type == 'roto':
        baseloc = userData['nim_shotPath'] + "/pub/" + element_type
    elif element_type == 'comp':
        baseloc = userData['nim_compPath']
    elem = nimFile.elementType_toAbbrev( element_type )
    ver   = int(info['versionNumber'])
    # task  = PS.knob('nim_task').value()
    task  = 'conform'
    ext   = 'exr'

    path = nimRt.publishOutputPath( baseloc, shot, name, ver, task, elem=elem, ext=ext, isseq=True, format='nim', force_posix=True)

    return path

def renderVersionAndPublishReserve( info, userdata ):
    '''
    Reserve render version and publish
    This function will reserve a publish version slot for the render and log it into the publishing system
    If start and end frames are provided then they will be used for  the render publish to stablish the start and end
    of the render in the system

    Please refer to nim_core.nim_rohtau.pubPath() for more info
    ...
    -------
    dict
        Dictionary from nim_rohtau.pubPath() with filename, version, fileId and elementID. If any error, empty dict or False
    '''

    '''
    User Data
    {'audioElementID': '4',
    'batchOpenClipElementID': '1',
    'batchTaskTypeFolder': 'tasks/comp',
    'batchTaskTypeID': '9',
    'currentShotID': '640',
    'nim_compPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040/out/2d',
    'nim_export_sequence': True,
    'nim_export_type': 'rohtauExportSequence',
    'nim_jobID': 45,
    'nim_jobName': 'test_21019',
    'nim_jobPath': '/jobs/test_21019',
    'nim_platesPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040/in/plates',
    'nim_renderPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040/out/cg',
    'nim_serverID': '29',
    'nim_serverOSPath': '/jobs',
    'nim_shotPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040',
    'nim_showID': '55',
    'nim_userID': 7,
    'openClipElementID': '1',
    'pub_autover': 'Manual',
    'pub_dopub': True,
    'pub_ver': 2,
    'shotData': {'SCF_040': {'video': {'compPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040/out/2d',
                                        'jobPath': '/jobs/test_21019',
                                        'nim_iconPath': '/tmp/test_21019/work/SCF/SCF_040/in/plates/main/SCF_040__conform_plates__main/v001/SCF_040__conform_plates__main__v001.1001.exr',
                                        'nim_shotID': '640',
                                        'platesPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040/in/plates',
                                        'renderPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040/out/cg',
                                        'resolvedPath': 'test_21019/work/SCF/SCF_040/in/plates/main/SCF_040__conform_plates__main/v001/SCF_040__conform_plates__main__v001.[1001-1070].exr',
                                        'shotPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040',
                                        'success': True}}},
    'videoElementID': '4'}
    '''
    ...
    'assetName': 'Wide_B',
    'assetType': 'video',
    'channelsEncoding': 'Float',
    'colourSpace': 'ACEScg',
    'depth': '16-bit fp',
    'destinationHost': 'vxfhost',
    'destinationPath': '/tmp',
    'drop': False,
    'fps': '23.976',
    'handleIn': 10,
    ...
    'height': 1080,
    'isBackground': False,
    'namePattern': '<nim_shot_plates>/<track name>/<shot '
                    'name>__conform_plates__<track name>/<version name>/<shot '
                    'name>__conform_plates__<track name>__<version '
                    'name>.<frame><ext>',
    'nbChannels': 3,
    'pixelLayout': 'RGB',
    'recordIn': 86470,
    'recordOut': 86520,
    'resolvedPath': '/jobs/test_21019/test_21019/work/SCF/SCF_040/in/plates/conform/SCF_040__conform_plate__main/v001/SCF_040__conform_plate__main__v001.####.exr',
    'scanFormat': 'PROGRESSIVE',
    'segmentIndex': 4,
    'sequenceDrop': False,
    'sequenceFps': '23.976',
    'sequenceName': 'Simple_5sec_conform.Exported.01',
    'shotName': 'SCF_040',
    'sourceIn': 870766,
    'sourceOut': 870836,
    'tapeName': 'A020_C002_0729M6',
    'track': '001',
    'trackName': 'main',
    'versionName': 'v001',
    'versionNumber': 1,
    ...
    if not int(userdata['pub_dopub']):
        nimP.warning("Shot %s won't be publishded in NIM"%info['shotName'])

    res    = False
    curver = int(info['versionNumber'])
    newver = curver
    ...
    # Pub Info
    shot_info = userdata['shotData'][info['shotName']][info['assetType']]
    parent    = 'SHOT'
    parentID  = int(shot_info['nim_shotID'])
    path      = info['resolvedPath']
    path      = path.replace('%04d', '####') # Fix Nuke's padding format
    basename  = os.path.basename(path)
    # Set Render Version
    if userdata['pub_autover'] == 'Up Version':
        # Up Version
        newver   = nimRt.getNextPublishVer ( basename, parent, parentID )
        if newver == 1:
            nimP.info("No version published yet for: %s"%basename)
        else:
            nimP.info("Version updated to %d, was %d in %s"%(newver, curver))
    else:
        # Manual
        # Just use whatever version is set in Version parameter
        newver = int(userdata['pub_ver'])
        pass

    # Publish render
    myuser        = nimUtl.get_nim_user()
    userid        = nimUtl.getuserID(myuser)
    comment       = ""
    verpermission = nimRt.pubOverwritePolicy.ALLOW_ALL
    source_fileid = 0
    startframe    = 1001 # Assume convention of all shots starting at 1001
    duration      = ( info['sourceOut'] - info['sourceIn'] ) - 1
    ...
    kwargs = {
        ...
        'end':           endframe,
        'handles':       0,
        'overwrite':     verpermission,
        'state':         nimRt.pubState.PENDING,
        'require_task':  True,
        'task_status':   nimRt.taskStatusID.IN_PROGRESS,
        'verbose':       True
    }
    # TODO: New conform is not assigned to user.
    # TODO: create render in NIM
    res = nimRt.pubPath( *args, **kwargs)
    ...
    'fileID': 14001,
    'filename': 'SCF_040__conform_plate__main__v001.####.exr',
    'filepath': '/jobs/test_21019/test_21019/work/SCF/SCF_040/in/plates/conform/SCF_040__conform_plate__main/v001/SCF_040__conform_plate__main__v001.####.exr',
    'msg': None,
    'renderID': None,
    'success': True,
    'version': 1}
...
def nimCreateShot(nim_showID, info, userdata) :
    ...
    if nim_showID != None and info['shotName']: 
        nim_shotID = False
        ...
        nimP.info("Exporting Shot Info: %s"%info['shotName'])
        # TODO: This is not setting Head and Tails for the shot correctly
        # Duration should always be the whole clip
        # Also we can probably get infor about focus lens, etc , from the clip
        # metadata
        ...
        if shotInfo['success'] == 'true':
            ...
            nimP.info("Shot ID: %s" % nim_shotID)
            if 'error' in shotInfo:
                nimP.warning("Can't get shot %s info  %s" % (nim_shotName, shotInfo['error']))
        else:
            result['success'] = False
            nim_shotID = False
            if shotInfo['error']:
                #error exists
                nimP.Error("Can't create shot %s info  %s" % (nim_shotName, shotInfo['error']), openwindow=True)
        ...
        if nim_shotID == False:
            ...
            if nim_assetType == 'video' :
                ...
                nimP.info("Shot %s icon path: %s" % (nim_shotName, result['nim_iconPath']))

            else :
                nimP.info('Skipping icon upload for non-video asset type: %s'%nim_assetType)

        if not nim_shotID:
            nimP.error("Something went wrong when creating/updating shot %s. Can't get a valid ShotID"%nim_shotName)
            return False

        # TODO: add all output paths
        pathinfo = nimAPI.get_paths(item='shot', ID=int(nim_shotID))
        locskeys = ('jobPath', 'shotPath', 'platesPath', 'renderPath', 'compPath')
        for loc in locskeys:
            result[loc] = ""
        if not pathinfo :
            nimP.error( 'No Path Information found in the NIM API!: %s'%nim_shotName )
            return False
        else:
            serverpath = userdata['nim_serverOSPath'] + '/' + userdata['nim_jobName']
            #  Set Comp Path :
            if pathinfo and type(pathinfo)==type(dict()) and 'comps' in pathinfo :
                compPath=os.path.normpath( os.path.join( serverpath, pathinfo['comps'] ))
                result['compPath'] = compPath
            
            #  Set Render Path :
            if pathinfo and type(pathinfo)==type(dict()) and 'renders' in pathinfo :
                renderPath=os.path.normpath(os.path.join( serverpath, pathinfo['renders'] ) )
                result['renderPath'] = renderPath

            #  Set Plates Path :
            if pathinfo and type(pathinfo)==type(dict()) and 'plates' in pathinfo :
                platesPath=os.path.normpath(os.path.join( serverpath, pathinfo['plates'] ) )
                result['platesPath'] = platesPath

            # Shot/Asset path
            if pathinfo and type(pathinfo)==type(dict()) and 'root' in pathinfo :
                shotPath=os.path.normpath( os.path.join( serverpath, pathinfo['root'] ) )
                result['shotPath'] = shotPath

            jobnumber = userdata['nim_jobName']
            # jobpath = os.path.normpath(os.path.join(self.server(), jobnumber ))
            jobpath = serverpath
            # self.set_jobPath( jobPath=jobpath )
            result['jobPath'] = jobpath

        result['nim_shotID'] = nim_shotID
        result['resolvedPath'] = nimResolvePath(nim_shotID=nim_shotID, keyword_string=nim_resolvedPath)

    elif not info['shotName']:
        nimP.error("Error exporting clip %s. Shot not set for that clip"%info['shotName'])
        return False
